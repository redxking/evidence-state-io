...
| P1-05 | Multi-source composition | P1-01, one adapter | Overlap cannot inflate coverage; unknown overlap is conservative; contradictions remain explicit. | [ADR-0015](adr/0015-multi-source-composition.md) accepted for `CORROBORATION`, implemented in envelope schema `1.1` against synthetic sources, and measured by the packaged `EmptyBench-P1-composed` benchmark (6/6 pairs discriminated); `PARTITION` and `OPTIONAL` deferred, owner decision outstanding for those |
